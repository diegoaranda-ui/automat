...
- **Finance Suite** (`finanzas.html`, the product): 3 AI-assisted modules that turn financial documents into structured working papers (papeles de trabajo) — Conciliación Bancaria (cartola vs libro mayor; incluye el **Agrupador de transferencias sueltas**: sube UN Excel con hojas Extracto+Mayor, conciliador Banco Internacional: agrupa el extracto por **Detalle + Fecha** (`fechaKey`) y suma; el **cruce contra el Mayor lo hace enteramente la IA por LOTES** (`cruzarConIA`, batch de 40 grupos, Sonnet, con `IA_MARCO`) — empareja cada grupo con la Nota del mismo tercero por nombre (variantes/abreviaturas/RUT), sin match por código (se retiró el determinista Nota/Fecha+Monto para bajar líneas y evitar el corte "documento muy grande"). Corre automático al cargar; el batching evita el `max_tokens` con alto volumen. Origen reportado = la Nota; `dif` = |total extracto| − |total de la Nota|. Dos entradas: subir Excel (`processConciliacionAgrupador`) o **leer la planilla** (`cruzarDesdePlanilla`→`leerConciliadorBanco` lee las hojas "Extracto"/"Mayor" de `CONCIL_SHEET_ID`). `computeAgrupador`/`analyzeAgrupador`/`saveAgrupadorToSheet` en `finanzas.html` (key `agrup`, resultados `#results-agrup`, copy 📋 por monto); escribe la pestaña **"Match"** vía `writeAgrupadorSheet` (visuales Kavak + filtro nativo). Detección flexible de hojas/columnas por nombre o contenido), **Control de Provisiones** (NetSuite GL vs monthly provisions), and **Impuesto Transferencia ICAR** (account 2801-01: per-Stock-ID cross of customer-invoiced transfer tax vs ICAR fund deductions; deterministic like Provisiones — classification ok / invoiced-not-deducted (wait for ICAR rendition) / negative-result-to-book (liability account: Importe = Crédito − Débito, so facturado is positive and descontado negative; per-stock sum < 0 = deductions exceed billing → feeds the analyst's adjustment entry; the account balance is the plain sum of all Importe and must tie to the trial balance — the module analyzes, never books; this loss is STRUCTURAL and expected: the customer is charged 1.5% of the agreed sale price while ICAR deducts 1.5% of the fiscal appraisal (new-car base), usually higher for used cars — the project's goal is to book it efficiently/precisely, NOT to reduce it; the monthly pivot doubles as the management figure of how much is lost per month, but that is a byproduct, never the focus); writes to the team's own spreadsheet `ICAR_SHEET_ID` tab 'Análisis ICAR'). ICAR also has the Desde/Hasta accounting-date filter (same pattern as Provisiones: keeps raw `filas` with a `ymd` from Período month + Fecha/Date day, re-aggregates per Stock ID via `aggregateIcarFilas(filas, hasMesV, ymdDesde, ymdHasta)` on each analysis; `icar-desde`/`icar-hasta` inputs; `periodo` shown in the cuadratura banner and sheet meta — narrowing the window pushes deductions whose invoice falls outside it into the `sinFactura` "fuera de ventana" bucket, never a false "por registrar"). The legacy Supply inspection tools (DocScan/B2B/validador) were removed in Jul 2026; the Reportería, Gestión Documental and Automatización Contable modules were removed in Jul 2026 to focus the suite on the deterministic month-close tools.
...
4. **Date-range filter (Desde/Hasta):** the module keeps the raw parsed rows (`filas`, with a `ymd` accounting date built from Período month + Date day) and re-aggregates on each analysis via `aggregateProvisionesFilas(filas, tokenIdx, anio, ymdDesde, ymdHasta)`, which overwrites `f.movs`/`f.otros` so drill-down, comparativa and "otros" all reflect the window. Two `<input type=date>` (`prov-desde`/`prov-hasta`) bound the accounting date; `mesDesde`/`mesCierre` (= months of the range ends) drive the matrix columns and loops. `mesCierre` is the actionable (closing) month; advancing the "Hasta" day week by week gives weekly progress cuts (providers whose current-month invoice hasn't arrived yet surface as actionable). `mes_desde`/`mes_cierre` travel in the result and the sheet payload; `writeProvisionesSheet`/`writeProvisionesDashboard_` use `nMes = mesCierre - mesDesde + 1` for all column widths/offsets.
5. **Actionable focus:** a provider is actionable for mes_cierre ONLY if it has no factura/provision in the closing month AND invoiced in the immediately previous month (`accionable` flag + `ultimo_mes_factura`). Recurrent providers missing the closing month but inactive for months go to a "confirmar vigencia" bucket (no automatic provision suggested). Earlier-month gaps are reference. UI and Sheet split pendings into "ACCIONABLE <mes>" / "Revisar vigencia" / "Referencia"; a deterministic monthly comparison dashboard (per-provider invoice evolution + MoM deltas, ⚠ at ±50% or missing invoice) renders in the UI and is written to the sheet via the `comparativa` payload.
